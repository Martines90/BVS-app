...
const ConstantsTable = () => {
  ...
  const loadConstants = async () => {
    ...
    _contentCheckConstants.VOTING_CHECK_ASKED_NUM_OF_QUESTIONS = `${await asyncErrWrapper(
      async () => Number(await contract?.VOTING_CHECK_ASKED_NUM_OF_QUESTIONS() || 0)
    )() || 0}`;

    _contentCheckConstants.ARTICLE_CHECK_ASKED_NUM_OF_QUESTIONS = `${await asyncErrWrapper(
      async () => Number(await contract?.ARTICLE_CHECK_ASKED_NUM_OF_QUESTIONS() || 0)
    )() || 0}`;

    _contentCheckConstants.ARTICLE_RESPONSE_CHECK_ASKED_NUM_OF_QUESTIONS = `${await asyncErrWrapper(
      async () => Number(await contract?.ARTICLE_RESPONSE_CHECK_ASKED_NUM_OF_QUESTIONS() || 0)
    )() || 0}`;
